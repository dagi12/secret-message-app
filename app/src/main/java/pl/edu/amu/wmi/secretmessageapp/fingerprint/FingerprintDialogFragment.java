 ...
 */
@EFragment(value = R.layout.fingerprint_dialog_container)
public class FingerprintDialogFragment extends DialogFragment implements AuthCallback {
    ...
    @ViewById(R.id.use_fingerprint_in_future_check)
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        fingerprintViewModel.setAuthCallback(this);
        setCancelable(false);
    }
    ...
    private void verifyPassword() {
        if (!fingerprintViewModel.checkPassword(mPassword.getText().toString())) {
            return;
        }
        if (stage == Stage.NEW_FINGERPRINT_ENROLLED) {
            boolean futureFingerprint = mUseFingerprintFutureCheckBox.isChecked();
            fingerprintViewModel.passwordVerified(futureFingerprint);
            if (futureFingerprint) {
                stage = Stage.FINGERPRINT;
            }
        }
        configActivity.onAuthenticated(false);
        mPassword.setText("");
        dismiss();
    }
    ...
    public void onAuthenticated() {
        ...
        fingerprintStatus.setTextColor(getResources().getColor(R.color.success_color, null));
        fingerprintStatus.setText(getString(R.string.fingerprint_success));
        fingerprintIcon.postDelayed(() -> {
            // FingerprintCallback from FingerprintViewModel. Let the activity know that authentication was
            // successful.
            configActivity.onAuthenticated(true);
            dismiss();
